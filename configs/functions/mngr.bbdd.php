...
if ( ! function_exists('qp_esc'))
{
	...
	function qp_esc ($valor = '', $or_null = FALSE, mysqli $conection = NULL, $f_as_f = FALSE){
		...
		if (is_string($valor) and preg_match('/^('.implode('|', $_functions_alws).')(\(\))?$/i', $valor))  ## Palabras Reservadas No Peligrosas
		{
			return $valor;
		}
		elseif (is_string($valor) and preg_match('/\b('.implode('|', $_functions).')\b/i', $valor) and $f_as_f)  ## Palabras Reservadas
		{
